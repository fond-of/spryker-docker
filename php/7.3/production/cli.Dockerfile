...
FROM $REGISTRY/$IMAGE_NAME:7.3-cli
...
RUN sed -i -e 's/\"REPLACE_WITH_REAL_KEY\"/\${NEW_RELIC_LICENSE_KEY}/' \
  ...
  -e 's/\;newrelic.loglevel\s=\s.*/newrelic.loglevel="error"/' \
  -e 's/\;newrelic.transaction_tracer.detail\s=\s.*/newrelic.transaction_tracer.detail=\${NEW_RELIC_TRACE_DETAIL}/' \
...
COPY ./docker-entrypoint/production/cli.sh /usr/local/bin/docker-entrypoint.sh
